...
export class NumberDeviator<I, O extends number, E> extends BaseDeviator<I, O, E> {
  ...
  public greater(value: number) {
    return this.then(input => input > value ? ok(input) : err("greater" as const));
  }
  ...
  public less(value: number) {
    return this.then(input => input < value ? ok(input) : err("less" as const));
  }
  ...
  public max(value: number) {
    return this.then(input => input <= value ? ok(input) : err("max" as const));
  }
  ...
  public min(value: number) {
    return this.then(input => input >= value ? ok(input) : err("min" as const));
  }
   ...
   */
  public negative() {
    return this.then(input => input < 0 ? ok(input) : err("negative" as const));
  }
   ...
   */
  public positive() {
    return this.then(input => input > 0 ? ok(input) : err("positive" as const));
  }
}
